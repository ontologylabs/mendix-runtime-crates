...
* `Dockerfile`: model-agnostic; JDK 11 base (`eclipse-temurin:11-jdk-jammy`);
  downloads the Mendix 8 build toolchain from the official CDN at build
...
* Pre-creates `$HOME/.local/share/Mendix` for the MX 11-origin `.NET` mxbuild whitelist
  FailFast (same fix as aide-mxtools).
* Default Mendix version: 8.18.35.97 (override via `--build-arg MENDIX_VERSION`).

### Verification status
* CDN source `mxbuild-8.18.35.97.tar.gz`: **HTTP 200 verified (2026-06-23)**.
...
validate a Mendix 8 app, producing the `.mda` the runtime crate runs.
