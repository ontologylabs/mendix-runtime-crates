...
> any Mendix 8.x application. Bind-mount your unzipped MDA, set a few env
...
The base image is `eclipse-temurin:11-jre-jammy`. **Mendix 8.18 LTS is certified
on Java 8 and Java 11**; Java 11 is used here (Mendix Cloud's late-MX8 default; it
runs Java-8 bytecode). For an app that pins Java 8, rebuild on
`eclipse-temurin:8-jre-jammy` via `--build-arg`. Mendix 8 runtime files are
downloaded from `https://cdn.mendix.com/runtime/mendix-${VERSION}.tar.gz` during
build.

> **Mendix 8 is out of standard support.** The final 8.18 LTS patch
> (`8.18.35.97`, this crate's default) is still served from the public CDN, so it
> builds with no extra steps. Older MX8 patches are **not** all CDN-hosted — for
> those, see [`PORTAL-DOWNLOAD.md`](PORTAL-DOWNLOAD.md).
