#!/bin/bash
# Mendix 11 Runtime Crate — entrypoint
#
# Boot sequence:
...
if [ -n "${MICROFLOW_CONSTANTS:-}" ] && [ "${MICROFLOW_CONSTANTS}" != "{}" ]; then
    ...
    if echo "${MICROFLOW_CONSTANTS}" | jq -e . >/dev/null 2>&1; then
        CONSTANTS_OPTIONAL_FIELD=",\"MicroflowConstants\":${MICROFLOW_CONSTANTS}"
        echo "Applying ${MICROFLOW_CONSTANTS_KEY_COUNT:-N} constant overrides via MICROFLOW_CONSTANTS env."
    else
