# Mendix 11 Runtime Crate

> A model-agnostic, app-agnostic, version-pinned Docker image for running
> any Mendix 11.x application. Bind-mount your unzipped MDA, set a few env
...
|---|---|---|
| `ontologylabs/mendix-runtime:11.6.4` | Exact version (immutable) | Production, repeatable builds |
| `ontologylabs/mendix-runtime:11` | Latest 11.x in this crate's release stream | Dev, demo, easy upgrades |

Future versions:

* `ontologylabs/mendix-runtime:10.24.x` — Mendix 10 LTS (planned)
* `ontologylabs/mendix-runtime:9.x` — Mendix 9 (planned)

## Build

```bash
cd "$(git rev-parse --show-toplevel)"/delivery/crates/mendix-11   # or: cd <delivery-root>/crates/mendix-11
docker build \
...
The base image is `eclipse-temurin:21-jre-jammy`. Mendix 11 runtime files
...
docker run -d \
    ...
    -v claudius-data:/opt/mendix/data \
