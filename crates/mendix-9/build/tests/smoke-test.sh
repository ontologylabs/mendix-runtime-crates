#!/usr/bin/env bash
# Smoke test for the Mendix 9 build crate.
#
# Compiles a Mendix 9 project (.mpr) into an .mda using the build-crate image,
...
MENDIX_VERSION="${MENDIX_VERSION:-9.24.20.33307}"
IMAGE="${BUILD_IMAGE:-ontologylabs/mendix-mxbuild:${MENDIX_VERSION}}"

MPR_PATH="${1:-}"
if [ -z "${MPR_PATH}" ] || [ ! -f "${MPR_PATH}" ]; then
    echo "[FATAL] Pass the path to a Mendix 9 .mpr as the first argument."
