# Mendix 9 Build Crate

> A model-agnostic, version-pinned Docker image that compiles **any** Mendix 9.x
...
This is the **build** companion to the [`mendix-9` runtime crate](../). The build
...
or a contributor with only Docker can compile a Mendix 9 app without Studio Pro.
...
| Tag | Pins | Use |
|---|---|---|
| `ontologylabs/mendix-mxbuild:9.24.20.33307` | Exact version (immutable) | Reproducible CI |
| `ontologylabs/mendix-mxbuild:9` | Latest 9.x in this crate | Dev, demo |

## Build the image

```bash
cd crates/mendix-9/build
docker build \
    --platform linux/amd64 \
    --build-arg MENDIX_VERSION=9.24.20.33307 \
    -t ontologylabs/mendix-mxbuild:9.24.20.33307 .
# mxbuild + mx are pulled from cdn.mendix.com/runtime/mxbuild-9.24.20.33307.tar.gz at build.
```
...
docker run --rm \
    ...
    ontologylabs/mendix-mxbuild:9.24.20.33307 \
