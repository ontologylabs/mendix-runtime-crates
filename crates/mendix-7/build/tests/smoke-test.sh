#!/usr/bin/env bash
# Smoke test for the Mendix 7 build crate.
#
# Compiles a Mendix 7 project (.mpr) into an .mda using the build-crate image,
...
MENDIX_VERSION="${MENDIX_VERSION:-7.23.8.58888}"
IMAGE="${BUILD_IMAGE:-ontologylabs/mendix-mxbuild:${MENDIX_VERSION}}"

MPR_PATH="${1:-}"
if [ -z "${MPR_PATH}" ] || [ ! -f "${MPR_PATH}" ]; then
    echo "[FATAL] Pass the path to a Mendix 7 .mpr as the first argument."
