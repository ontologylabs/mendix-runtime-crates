# Mendix 10 Runtime Crate

> A model-agnostic, app-agnostic, version-pinned Docker image for running
> any Mendix 10.x application. Bind-mount your unzipped MDA, set a few env
...
|---|---|---|
| `ontologylabs/mendix-runtime:10.24.13.86719` | Exact version (immutable) | Production, repeatable builds |
| `ontologylabs/mendix-runtime:10.24` | Latest 10.24 LTS patch in this crate | Pinned LTS line |
| `ontologylabs/mendix-runtime:10` | Latest 10.x in this crate's release stream | Dev, demo, easy upgrades |
...
```bash
cd "$(git rev-parse --show-toplevel)"/delivery/crates/mendix-10   # or: cd <delivery-root>/crates/mendix-10
docker build \
    --platform linux/amd64 \
    --build-arg MENDIX_VERSION=10.24.13.86719 \
    -t ontologylabs/mendix-runtime:10.24.13.86719 \
    -t ontologylabs/mendix-runtime:10.24 \
    -t ontologylabs/mendix-runtime:10 \
    .
```

The base image is `eclipse-temurin:21-jre-jammy` (Mendix 10.24 LTS supports
JDK 21). Mendix 10 runtime files are downloaded from
