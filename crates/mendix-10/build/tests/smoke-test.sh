#!/usr/bin/env bash
# Smoke test for the Mendix 10 build crate.
#
# Compiles a Mendix 10 project (.mpr) into an .mda using the build-crate image,
...
MENDIX_VERSION="${MENDIX_VERSION:-10.24.13.86719}"
IMAGE="${BUILD_IMAGE:-ontologylabs/mendix-mxbuild:${MENDIX_VERSION}}"

MPR_PATH="${1:-}"
if [ -z "${MPR_PATH}" ] || [ ! -f "${MPR_PATH}" ]; then
    echo "[FATAL] Pass the path to a Mendix 10 .mpr as the first argument."
